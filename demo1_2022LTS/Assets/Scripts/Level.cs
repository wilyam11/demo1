...
public class Level : MonoBehaviour
{
    ...
    new Vector3(Input.mousePosition.x, Input.mousePosition.y, Camera.main.nearClipPlane)
        ...
        // 遊戲結束？
        if (State == LevelState.Playing & _notZero == notes.Length & finishNotes.Count == 0)
        {
            Debug.Log("Game Complete!");
            State = LevelState.Result;
            StartCoroutine(GameComplete(2f));
        }
    }
    ...
    private IEnumerator GameComplete(float delay)
    {
        yield return new WaitForSeconds(delay);

        GamePlay.ShowCompleteWindow((int)Score, Combo, scoreData);
    }
}